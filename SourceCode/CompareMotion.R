...
df_motions = read.xlsx(file.path(usedir,'SourceData.xlsx'),'EDFig4')
...
Para.names = c('translation','rotation')
...
fig = ggarrange(p[[1]], p[[2]], 
                        common.legend = T,
                        legend = 'none',
                        ncol = 2, nrow = 1, align = "hv")

pdf(NULL)
pdf(file.path(figDir,'EDFig4.pdf'), 9, 4)
print(fig)
dev.off()
...
for(i in 1:nParas){
  ...
  lme1 = lmer(value ~ Time + Subs + (1 | Subs), data = use.df)
  print(summary(lme1))
  anova(lme1)
